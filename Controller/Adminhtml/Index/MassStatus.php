    ...
    public function execute()
    {
        try {
            ...
            foreach ($pressItems as $item) {
                $item->setData('is_active',
                    $this->getRequest()->getParam('is_active') == '1' ? Press::STATUS_ENABLED : Press::STATUS_DISABLED
                )->save();
            }
            $this->messageManager->addSuccess(__('A total of %1 record(s) have been updated.', count($pressItems)));

        } catch (LocalizedException $e) {
            $this->messageManager->addError($e->getMessage());
        } catch (Exception $e) {
            $this->_getSession()->addException($e, __('Something went wrong while updating the press release(s) status.'));
        }
